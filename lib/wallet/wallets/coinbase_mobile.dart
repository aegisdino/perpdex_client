...
/// Coinbase Wallet 모바일 딥링크 구현
/// Web에서는 coinbase_wallet.dart가 사용되고,
...
class CoinbaseWallet extends EthereumWallet {
  ...
  void _handleDeepLink(Uri uri) {
    debugPrint('Coinbase deeplink received: $uri');

    if (uri.path.contains('/coinbase/callback')) {
      final params = uri.queryParameters;

      // 에러 처리
      if (params.containsKey('error')) {
        final error = params['error'];
        debugPrint('Coinbase error: $error');
  ...
  String _createDeepLink({
    ...
    final returnUrl = kIsWeb
        ? Uri.base.toString()
        : 'bitupdownapp://coinbase/callback';
  ...
  Future<String?> connect() async {
    try {
      ...
      debugPrint('Opening Coinbase: $deepLink');
      ...
      debugPrint('Coinbase connect error: $e');

      // Coinbase이 설치되지 않은 경우 앱스토어로 이동
      if (e.toString().contains('No application found')) {
        await openUrl(
          'https://coinbase.app/download',
          target: '_blank',
        );
      }
      rethrow;
  ...
  Future<void> switchChain(String chainId) async {
    ...
    throw UnimplementedError(
      ...
      'Please switch network in Coinbase app.',
    );
  }
  ...
  Future<void> addNetwork({
    ...
    throw UnimplementedError(
      ...
      'Please add network in Coinbase app.',
    );
  }
