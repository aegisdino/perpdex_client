...
/// Trust 모바일 딥링크 구현
/// Web에서는 trust_ethereum_wallet.dart가 사용되고,
...
class TrustWallet extends EthereumWallet {
  ...
  void _handleDeepLink(Uri uri) {
    debugPrint('Trust deeplink received: $uri');

    if (uri.path.contains('/trust/callback')) {
      final params = uri.queryParameters;

      // 에러 처리
      if (params.containsKey('error')) {
        final error = params['error'];
        debugPrint('Trust error: $error');
  ...
  Future<String?> connect() async {
    try {
      ...
      debugPrint('Opening Trust: $deepLink');
      ...
      debugPrint('Trust connect error: $e');

      // Trust이 설치되지 않은 경우 앱스토어로 이동
      if (e.toString().contains('No application found')) {
        await openUrl(
          'https://trust.app/download',
          target: '_blank',
        );
      }
      rethrow;
  ...
  Future<void> switchChain(String chainId) async {
    ...
    throw UnimplementedError(
      ...
      'Please switch network in Trust app.',
    );
  }
  ...
  Future<void> addNetwork({
    ...
    throw UnimplementedError(
      ...
      'Please add network in Trust app.',
    );
  }
